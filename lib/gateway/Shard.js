...
class Shard extends EventEmitter {
    ...
    */
    ...
    wsEvent(packet) {
        if(packet.d.guild_id && this._isGuildFiltered(packet.d.guild_id)) {
            return;
        }
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_CREATE": {
                if(this._isGuildFiltered(packet.d.id)) {
                    return;
                }
            ...
            case "READY": {
                ...
                packet.d.guilds.forEach((guild) => {
                    if(guild.unavailable) {
                        ...
                        if(!this._isGuildFiltered(guild.id)) {
    ...
    _isGuildFiltered(id) {
        const filterGuilds = this.client.options.filterGuilds;
        return filterGuilds.length > 0 && !filterGuilds.includes(id);
    }
